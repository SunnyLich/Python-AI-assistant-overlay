...
- ChromaDB-backed semantic memory and background memory LLM jobs are always
  explicit opt-ins on macOS, even when `WISP_MACOS_SAFE_MODE=0`; memory falls
  back to plain JSON storage by default. Set
