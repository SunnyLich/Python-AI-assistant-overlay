...
| Settings | shared `.env` config, models, callers, voice, memory, chat, UI knobs, OS-keychain API keys | Qt `SettingsDialog` | Swift `SettingsPanel` with LLM/TTS tests, caller editing, shared UI keys, and API key status/save/clear via the Python sidecar |
