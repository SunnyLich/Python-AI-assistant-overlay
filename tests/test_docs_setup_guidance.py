...
class DocsSetupGuidanceTests(unittest.TestCase):
    ...
    def test_readme_points_to_preflight_after_setup(self) -> None:
        readme = (ROOT / "README.md").read_text(encoding="utf-8")

        self.assertIn(r".\.venv\Scripts\python.exe scripts\check_dev_environment.py", readme)
        self.assertIn(".venv/bin/python scripts/check_dev_environment.py", readme)
